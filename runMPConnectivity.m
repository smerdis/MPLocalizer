...
for iCond = 1:nConds
    ...
    t.Gen.wedgeRequests = 0:(t.Gen.wedgeDuration):...
        (t.Gen.wedgeDuration)*(p.Gen.numCycles-1);
...
% ------------------------------------------------------------------------
for iBlock = 1:p.Gen.numCycles % for each block, e.g. M, P, etc
...
if p.Gen.blankDuration(2) > 0
    ...
    vbl = Screen('Flip', win, t.Gen.trigger + p.Gen.blankDuration(1) + ...
        (t.Gen.wedgeDuration)*p.Gen.numCycles - ifi/2);
    t.Gen.endStim = vbl;

    % End of final blank period
    vbl = Screen('Flip', win, t.Gen.trigger + p.Gen.blankDuration(1) + ...
        (t.Gen.wedgeDuration)*p.Gen.numCycles + p.Gen.blankDuration(2) - ifi/2);
    t.Gen.endRun = vbl;
else
...
% Save file
if p.Gen.saveFile
    saveFile = sprintf('data/mpConnectivity_%s_run%02d_%s.mat', subjectID, run, datestr(now,'yyyymmdd'));
